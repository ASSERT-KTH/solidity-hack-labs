...
describe("attack unchecked_low_level_calls/0x663e4229142a27f00bafb5d087e1e730648314c3.sol", function () {
  ...
  async function deployContracts() {
    ...
    const PandaCallerSuccess = await ethers.getContractFactory("contracts/unchecked_low_level_calls/0x663e4229142a27f00bafb5d087e1e730648314c3_attack.sol:PandaCallerSuccess");
    const pandaCallerSuccess = await PandaCallerSuccess.connect(owner).deploy(contract.target);
    ...
    const MyERC721Success = await ethers.getContractFactory("contracts/unchecked_low_level_calls/0x663e4229142a27f00bafb5d087e1e730648314c3_attack.sol:MyERC721Success");
    ...
    const saleAuctionSuccess = await SaleClockAuction.connect(owner).deploy(nftSuccess.target, 10);
    ...
    return {pandaCaller, contract, saleAuction, siringAuction, nft, nftSuccess, saleAuctionSuccess, pandaCallerSuccess}
  };
  ...
  it('sanity check: unchecked_low_level_calls/0x663e4229142a27f00bafb5d087e1e730648314c3.sol in function withdrawAuctionBalances()', async function () {
    const {contract, saleAuctionSuccess, siringAuction} = await loadFixture(deployContracts);
    await expect(saleAuctionSuccess.connect(owner).transferOwnership(contract.target)).not.be.reverted;

    expect(await ethers.provider.getBalance(saleAuctionSuccess.target)).to.be.equal(amount);
    ...
    await contract.connect(owner).setSaleAuctionAddress(saleAuctionSuccess.target);

    await expect(contract.connect(owner).withdrawAuctionBalances()).to.not.be.reverted;

    expect(await ethers.provider.getBalance(saleAuctionSuccess.target)).to.be.equal(0);
  ...
  it("sanity check: unchecked_low_level_calls/0x663e4229142a27f00bafb5d087e1e730648314c3.sol in function giveBirth()", async function () {
    const {pandaCallerSuccess, contract, saleAuction, siringAuction} = await loadFixture(deployContracts);
    ...
    expect(await ethers.provider.getBalance(pandaCallerSuccess.target)).to.be.equal(0);
    await expect(pandaCallerSuccess.withdraw()).to.not.be.reverted;
    expect(await ethers.provider.getBalance(contract.target)).to.be.equal(ethers.parseEther("0.002"));
    expect(await ethers.provider.getBalance(pandaCallerSuccess.target)).to.be.equal(ethers.parseEther("0.998"));
    ...
    expect(await ethers.provider.getBalance(pandaCallerSuccess.target)).to.be.equal(amount);
